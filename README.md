...
The benchmark reports actual values from the current machine. The checked-in run uses the seeded expedition runtime (`seed=benchmark`, normal mode) in headless Chromium without assuming a discrete GPU: 7.33 FPS, 136.49 ms sampled frame time, 92 draw calls, 1,703 triangles, 51 resources, 3.46 MB encoded resource bytes, 96 geometries, and 21 textures. The run telemetry also records 4.30 ms world generation, 1.00 ms nav/collision build, 5.30 ms reset, 0.30 ms collider build, 1.50 ms module assembly, and 4.30 ms total world assembly. The in-app visual QA overlay is a separate measurement; neither environment is presented as a universal hardware claim.

The expedition entry now emits a 702.55 kB main JavaScript chunk (190.46 kB gzip); the legacy arena `world.js` is lazy-loaded as a separate 25.33 kB chunk (8.04 kB gzip). Floors, static wall/obstacle boxes, and module frames use renderer batching; the remaining main chunk size and draw-call count are known optimization targets, not hidden behind a made-up budget.
