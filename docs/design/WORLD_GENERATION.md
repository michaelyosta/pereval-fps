...
WorldValidator checks connectivity, objective and extraction reachability, route length, branching, connector validity, bounds overlap, point bounds, resources, and density. Failed attempts use a derived seed and stop after a bounded attempt count. GeneratedWorld and WorldAssembler keep logical generation independent from renderable objects; SpatialIndex provides nearby queries for later collision, raycast, noise, and enemy systems.
